...
public sealed class ExportRepositoryReportUseCase(
    IGetRepositoryAnalysisUseCase getRepositoryAnalysisUseCase,
    IReportPdfRenderer reportPdfRenderer) : IExportRepositoryReportUseCase
{
    public async Task<ReportFile?> GetAsync(string sourceCraftId, ReportFormat format, CancellationToken cancellationToken)
    {
        ...
        return format switch
        {
            ...
            ReportFormat.Pdf => new ReportFile("application/pdf", reportPdfRenderer.Render(analysis)),
