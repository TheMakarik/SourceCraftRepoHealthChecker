...
Тестовая страница: `http://localhost:8080/`. Одна HTML-страница без сборки, вшита в бинарник. На ней можно ввести PAT, войти через Яндекс ID, выбрать репозиторий из каталога, создать и удалить снапшот, вызвать любую категорию или все сразу. Ответ API показывается как есть. Для входа через Яндекс ID `YANDEX_REDIRECT_URI` должен указывать на эту страницу, например `http://localhost:8080/`.
